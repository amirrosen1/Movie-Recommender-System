...
int Test_3 ()
{
  std::string movie_name = "test";
  int movie_year = 1999;
  RecommenderSystem rs = RecommenderSystem ();
  sp_movie add_pointer = rs.add_movie (movie_name, movie_year, {1, 2, 3});
  sp_movie get_pointer = rs.get_movie (movie_name, movie_year);
  std::cout << "-------------------------" << std::endl;
  std::cout << "Test RecommenderSystem adder and getter pointer" << std::endl;
  if (add_pointer.get () != get_pointer.get ())
    {
      std::cerr << "Test RecommenderSystem adder and getter pointer failed."
                << std::endl;
      return EXIT_FAIL_TEST;
    }
  else
    {
      std::cout << "Test RecommenderSystem adder and getter pointer succeeded"
                << std::endl;
    }
  std::cout << "Test RecommenderSystem adder and getter pointer done"
            << std::endl;
  return EXIT_SUCCESS_TEST;
}
...
int Test_4 ()
{
  ...
  std::cout << "-------------------------" << std::endl;
  std::cout << "Test RecommenderSystem add movie for the movie" << std::endl;
  ...
  if (rs2.get_movie ("TheMagnificentAmbersons", 2001).get () != nullptr ||
    ...
    {
      std::cerr << "Test RecommenderSystem add movie for the movie failed."
                << std::endl;
      return EXIT_FAIL_TEST;
    }
  else
    {
      std::cout << "Test RecommenderSystem add movie for the movie succeeded"
                << std::endl;
    }
  std::cout << "Test RecommenderSystem add movie for the movie done"
            << std::endl;
...
int TestPCompilation_4 ()
{
  ...
  std::vector<RSUser> user = RSUsersLoader::create_users_from_file ("presubmit.in_u", std::move (rs));
  const RSUser &u0 = user[0];
  const RSUser &u1 = user[1];
  const RSUser &u2 = user[2];
  const RSUser &u3 = user[3];
...
int TestComparison_4 ()
{
  ...
  auto rs = RecommenderSystemLoader::create_rs_from_movies_file ("presubmit.in_m4");
  std::vector<RSUser> user = RSUsersLoader::create_users_from_file ("presubmit.in_u4", std::move (rs));
...
{
  auto rs = RecommenderSystemLoader::create_rs_from_movies_file ("presubmit.in_m5");
  std::vector<RSUser> user = RSUsersLoader::create_users_from_file ("presubmit.in_u5", std::move (rs));
...
{
  auto rs = RecommenderSystemLoader::create_rs_from_movies_file ("presubmit.in_m6");
  std::vector<RSUser> user = RSUsersLoader::create_users_from_file ("presubmit.in_u6", std::move (rs));
...
{
  auto rs = RecommenderSystemLoader::create_rs_from_movies_file ("presubmit.in_m7");
  std::vector<RSUser> user = RSUsersLoader::create_users_from_file ("presubmit.in_u7", std::move (rs));
...
int TestComparison_14 ()
{
  ...
  std::vector<RSUser> user = RSUsersLoader::create_users_from_file ("presubmit.in_u14", std::move (rs));
  ...
  if (first_prediction->get_name () != "TheMagnificentAmbersons"
    ...
    {
      std::cout << "Using RSUser recommendation function didn't return "
                   "expected output"
                << std::endl;
      return EXIT_FAIL_TEST;
    }
...
int TestComparison_15 ()
{
  try
    {
      auto rs = RecommenderSystemLoader::create_rs_from_movies_file ("presubmit.in_m15");
//				"presubmit_18.in_m");
...
//        rs->add_movie("TheyWereExpendable",1967,{1,2,8,5,9});
//        rs->add_movie("Harakiri",1967,{2,6,6,10,5});
      auto rs = RecommenderSystemLoader::create_rs_from_movies_file ("presubmit.in_m16");
      auto users = RSUsersLoader::create_users_from_file ("presubmit.in_u16", std::move (rs));
//				"presubmit_18.in_m");
      std::cout << "You didn't throw an exception when loading an invalid "
...
int main ()
{
  ...
  Test_Function additional_Tests[] = {
