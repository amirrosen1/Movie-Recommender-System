...
// Define a shared pointer to a user.
typedef std::shared_ptr<RecommenderSystem> ptr_to_rs;
...
class RSUser
{
 public:
   ...
   */
  RSUser (const std::string &user_name, const rank_map &user_rank_map,
          const ptr_to_rs &rs);

  /**
   * a getter for the user's name_
   * @return the username
   */
   ...
   * @param name the name_ of the movie
   * @param year the year_ the movie was created
